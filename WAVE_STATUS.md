...
| `web/app/dashboard/compute/center/page.tsx` | **DONE** | canonical compute center surface with queue/runtime observability driven by live compute endpoints |
...
| `tests/e2e/dashboard.test.ts` | **DONE** | Playwright full-stack route + stateful mutation coverage with fixture API (`11/11` local) |
...
| 29 | 2026-04-05 | Post-Wave stateful E2E + compute observability | Expanded Playwright dashboard coverage from route smoke to stateful mutation flows by persisting assumptions-family saves and compute-center run creation in the fixture API, bringing local E2E coverage to `11/11`. Added queue/runtime observability to the canonical compute center surface using the existing compute endpoints only: queued backlog, in-flight count, failed runs, last completed duration, selected output totals, and timed auto-refresh while active queue work exists. Also fixed strict-build nullability in dashboard/wizard App Router hooks and added a minimal Pages Router shim (`_document`, `_app`, `health`) so `next build` completes cleanly under Next 14. Re-verified with `npm --prefix api run build`, `npm --prefix web run build`, `npm test -- --runInBand` (`123/123`), `npm run test:e2e` (`11/11`), `python3 scripts/spec-compliance.py` (`9/9`, 0 warnings), and `git diff --check`. | compliant, 0 failures |
| 30 | 2026-04-05 | Post-Wave integration stabilization | Stabilized the locally integrated worker slices without reopening their core ownership: guarded compute queue heartbeat reads/writes behind `isAsyncComputeEnabled()` to stop fixture-backed Redis leakage, refreshed the E2E fixture SQL matchers for the expanded compute route selects, fixed the scope-review warning key typing issue, and made Playwright deterministic by disabling local server reuse and updating expectations to the current assumptions-save and random-UUID compute-run behavior. Re-verified with `npm --prefix api run build`, `npm --prefix web run build` (after a clean `.next` rebuild), `npm test -- --runInBand` (`123/123`), `npm run test:e2e` (`11/11`), `python3 scripts/spec-compliance.py` (`9/9`, 0 warnings), and `git diff --check`. | compliant, 0 failures |
